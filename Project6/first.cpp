...
#ifndef GLOBAL_SIZE_FACTOR
#define GLOBAL_SIZE_FACTOR      1024
#endif

#define NUM_ELEMENTS		NMB*GLOBAL_SIZE_FACTOR
...
main( int argc, char *argv[ ] )
{
	...
	fprintf( stderr, "%8d\t%4d\t%10d\t%10.3lf MegaMultsPerSecond\n",
		NMB*GLOBAL_SIZE_FACTOR, LOCAL_SIZE, NUM_WORK_GROUPS, (double)NUM_ELEMENTS/(time1-time0)/1000000. );
	
	ofstream outfile;
	outfile.open("result.csv",ios::app);
	outfile << NMB*GLOBAL_SIZE_FACTOR << ',' << LOCAL_SIZE << ',' << (double)NUM_ELEMENTS/(time1-time0)/1000000. << endl;
	outfile.close();
